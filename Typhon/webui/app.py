    ...
    if isinstance(value, str):
        ...
        try:
            ...
            if isinstance(p, list):
                return [str(x) for x in p if str(x)]
...
def _parse_ast(names: List[str]) -> List[type]:
    out = []  # type: List[type]
    for raw in names:
        name = raw[4:] if raw.startswith("ast.") else raw
        node = getattr(ast, name, None)
        if not isinstance(node, type) or not issubclass(node, ast.AST):
            raise ValueError(f"Invalid banned_ast: Unknown ast node '{raw}'")
        out.append(node)
    return out
...
def _common_params(data: Dict) -> Dict:
    ...
    return dict(
        ...
        banned_ast=_parse_ast(_parse_list(data.get("banned_ast"), "banned_ast")),
...
def run(host: str = "0.0.0.0", port: int = 6240) -> None:
